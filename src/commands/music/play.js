    ...
    async execute({ interaction }) {
        ...
        try {
            ...
            if (interaction.deferred)
                ...
                await interaction.reply({ content: 'Failed to join your voice channel.', flags: MessageFlags.Ephemeral });
            return;
        }
        // Resolve metadata
        let metas;
        try {
            ...
            if (interaction.deferred)
                ...
                await interaction.reply({ content: 'Failed to resolve that track.', flags: MessageFlags.Ephemeral });
            return;
        }
        if (!metas || metas.length === 0) {
            if (interaction.deferred)
                ...
                await interaction.reply({ content: 'No results.', flags: MessageFlags.Ephemeral });
            return;
        }
        ...
        if (final.length === 0) {
            if (interaction.deferred)
                ...
                await interaction.reply({ content: 'Could not find playable versions.', flags: MessageFlags.Ephemeral });
            return;
        }
        // Enqueue and report
        ...
        const summary = started ? `▶️ Started ${started} and queued ${queued}.`
