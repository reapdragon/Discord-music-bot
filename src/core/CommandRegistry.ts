...
export async function loadCommandClasses(): Promise<LoadedCmd[]> {
...
function printDuplicates(dupes: Map<string, string[]>) {
...
export async function registerCommands() {
  ...
  // Prepare body
  const body = loaded.map(({ instance }) => (instance.data as SlashCommand).toJSON());
...
export async function clearCommands(scope: 'global' | 'guild' = 'guild') {
  const rest = new REST({ version: '10' }).setToken(Config.token);
  if (scope === 'guild') {
    const gid = process.env.DEV_GUILD_ID;
    if (!gid) throw new Error('DEV_GUILD_ID not set.');
...
if (process.argv[1] && fileURLToPath(import.meta.url) === path.resolve(process.argv[1])) {
  ...
  (async () => {
    if (sub === 'register') await registerCommands();
    else if (sub === 'clear') await clearCommands((process.argv[3] as any) ?? 'guild');
    else console.log('Usage: register | clear [guild|global]');
  })().catch((err) => {
    console.error(err);
    process.exitCode = 1;
  });
