...
// ---------- Env check & logging ----------
function flag(v?: string | null) { return v && v.trim() ? '<set>' : '<unset>'; }

console.log('[boot] env:',
  'DISCORD_TOKEN=' + flag(process.env.DISCORD_TOKEN),
...
type ExecutableCmd = {
  ...
  execute(ctx: { interaction: ChatInputCommandInteraction }): Promise<void>;
};

const commands = new Collection<string, ExecutableCmd>();

async function findCommandsRoot(): Promise<{ root: string; ext: 'js' | 'ts' }> {
  ...
  try { await fs.access(distPath); return { root: distPath, ext: 'js' }; } catch {}
  try { await fs.access(srcPath);  return { root: srcPath,  ext: 'ts' }; } catch {}
  // Fallback to dist
...
async function loadCommands(): Promise<void> {
  ...
  async function walk(dir: string): Promise<string[]> {
    ...
    for (const it of items) {
      ...
      else if (it.isFile() && (it.name.endsWith(`.${ext}`))) out.push(full);
    }
    return out;
  }
  ...
  for (const file of files) {
    try {
      ...
      const instance: ExecutableCmd = new CmdClass();
      if (!instance?.data?.name || typeof instance.execute !== 'function') continue;
...
async function runSafely(interaction: ChatInputCommandInteraction, fn: () => Promise<void>) {
...
client.once('ready', async () => {
  ...
  try {
    if (!client.user?.id) throw new Error('client.user.id missing on ready');
